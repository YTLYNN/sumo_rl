...
class TrafficSignal:
    ...
    def update(self):
        ...
        if self.is_yellow and self.time_since_last_phase_change == self.yellow_time:
            ...
            #switch back new_phase

            self.green_phase = self.new_phase

            self.sumo.trafficlight.setRedYellowGreenState(self.id, self.all_phases[self.green_phase].state)


            self.next_action_time = self.env.sim_step + self.new_duration
            self.is_yellow = False


    def set_next_phase(self, action: int):

        duration_actions = [-1, -.75, -.50, -.25, 0, .25, .50, .75, 1]
        

        self.duration_action = duration_actions[action]
        self.new_phase = (self.green_phase + 1) if self.green_phase<len(self.green_phases)-1 else 0
        self.new_duration = self.green_phases[self.green_phase].duration * (1 + self.duration_action)
        
        if self.new_duration != 0:

            self.sumo.trafficlight.setRedYellowGreenState(
                self.id, self.all_phases[self.yellow_dict[(self.green_phase, self.new_phase)]].state
            )
            self.is_yellow = True
            self.time_since_last_phase_change = 0
        else:
            ...
            #self.new_phase = (self.green_phase + 1) if self.green_phase<len(self.green_phases)-1 else 0
            self.next_action_time = self.env.sim_step + 1

    def set_next_phase2(self, new_phase: int):
